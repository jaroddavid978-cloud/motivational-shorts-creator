...
async function getPixabayVideos(topic, count = 5) {
  try {
    // Try multiple search terms if topic doesn't return good results
    const searchTerms = [
      topic,
      'motivation',
      'success',
    ...
    for (const term of searchTerms) {
      const response = await axios.get('https://pixabay.com/api/videos/', {
        params: {
          ...
          q: term,
          per_page: count,
          orientation: 'vertical', // This ensures 9:16 videos
          video_type: 'film'
        }
      });
      
      if (response.data.hits.length > 0) {
        console.log(`   Found ${response.data.hits.length} vertical videos for "${term}"`);
...
// Helper: Generate script with Gemini - FIXED PROMPT
...

{
  ...
  "hashtags": ["#shorts", "#motivation", "#mindset"]
    ...
    // DYNAMIC fallback - different each time
    const fallbacks = [
      {
...
router.post('/create-video', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      videos: videos,
      isVertical: videos.length > 0 ? (videos[0].height > videos[0].width) : false,
