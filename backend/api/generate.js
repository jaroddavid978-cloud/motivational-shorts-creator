...
const generateLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 3,
  message: { success: false, error: 'Too many requests. Please wait 20 seconds before trying again.' }
});
...
// =============== MAIN GENERATION ROUTE with Rate Limiting ===============
router.post('/generate-script', generateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (err.response?.status === 429) {
      return res.status(429).json({ 
        success: false, 
        error: 'Rate limit reached. Please wait 20 seconds before trying again.' 
      });
    }
...
// =============== FIXED GEMINI HELPERS with Retry Logic ===============
