...
// Optional: force a specific model (must be visible to your key)
...
// Cache ListModels for 5 minutes to avoid rate/latency
const MODEL_CACHE_TTL_MS = parseInt(process.env.GEMINI_MODEL_CACHE_TTL_MS || '300000', 10);
let modelCache = { list: [], ts: 0 };

// Persistent store to prevent repeats (file-based, works on Render if disk is writable during build/runtime)
const STORE_PATH =
...
// Word count target for ~55–60s voiceover
const TARGET_MIN_WORDS = 135;
const TARGET_MAX_WORDS = 155;

// De-dup threshold for scripts
const SCRIPT_SIMILARITY_THRESHOLD = 0.82;

// Debug: log a preview of bad model outputs on parse failure
...
router.post('/generate-script', async (req, res) => {
  ...
  // Keep avoid lists small to save tokens
...
{"script":"...","titles":{"a":"...","b":"...","c":"..."},"description":"...","hashtags":["#shorts","..."]}`;
  ...
  try {
    // 1) Generate (JSON-coaxed via instruction, not schema)
    let genText = await callGeminiAuto(userPrompt, { temperature: 0.9, maxOutputTokens: 800 });
    let parsed = safeJsonParse(genText);

    // If still not JSON, retry once with stricter phrasing
    if (!parsed || !parsed.script) {
      if (DEBUG_MODEL_PREVIEW) console.log('[Gemini][parse-fail][gen] preview:', String(genText).slice(0, 300));
      const strictPrompt = `Return ONLY a single valid JSON object (no markdown, no code fences, no extra text) for the task below.

Task:
${userPrompt}`;
      genText = await callGeminiAuto(strictPrompt, { temperature: 0.85, maxOutputTokens: 800 });
    ...
    // 2) Word count check; refine once if out of range
...
Return ONLY this JSON object: {"script":"..."}.
...
${script}`,
      ...
      if (!refined?.script && DEBUG_MODEL_PREVIEW) {
    ...
    // 3) Titles + description: ensure present, short, unique, non-duplicate
...
- No emojis or hashtags.`,
    ...
    // Enforce distinct and non-duplicate titles again with fallbacks
    [titleA, titleB, titleC] = ensureThreeDistinct([titleA, titleB, titleC], topic, avoidTitles);

    // 4) Hashtags normalize to 10–12
    ...
    // 5) Final dedupe check on script; if too similar, ask for a variation once
...
${script}`,
    ...
    // 6) Store for future de-duplication
    pushScript(script);
    pushTitle(titleA);
    pushTitle(titleB);
    pushTitle(titleC);
...
// ======================================
// Gemini helpers (model discovery + call)
...
function scoreModel(shortName) {
  // Prefer flash for speed/cost, then pro for quality
  let s = 0;
...
async function orderedCapableModels() {
  ...
  // Put env-preferred first if available
...
async function callGeminiRaw(modelShortName, text, opts = {}) {
  // Only send fields the REST API accepts (no responseSchema/responseMimeType/systemInstruction here)
