...
{"script":"...","titles":{"a":"...","b":"...","c":"..."},"description":"...","hashtags":["#shorts","..."]}`;
  ...
  try {
    // 1) Generate
    const genText = await callGemini([userPrompt], { temperature: 0.9, maxOutputTokens: 800 });
    ...
    if (wordCount < TARGET_MIN_WORDS || wordCount > TARGET_MAX_WORDS) {
      const refineText = await callGemini(
        [
...
${script}`
    ...
    if (titlesMissing || titlesClash || !description) {
      const metaText = await callGemini(
        [
...
- No emojis or hashtags.`
    ...
    if (isNearDuplicateScript(script)) {
      const varyText = await callGemini(
        [
...
// ======================================
// Gemini call helper
async function callGemini(messagesOrText, opts = {}) {
  const text = Array.isArray(messagesOrText)
    ? messagesOrText.filter(Boolean).join('\n\n')
  ...
  const url = `${GEMINI_URL}/models/${encodeURIComponent(GEMINI_MODEL)}:generateContent?key=${encodeURIComponent(GEMINI_API_KEY)}`;
