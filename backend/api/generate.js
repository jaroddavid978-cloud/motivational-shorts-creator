...
// =============== Config ===============
// Env to set on Render: GEMINI_API_KEY
const GEMINI_API_KEY = process.env.GEMINI_API_KEY;

// Use v1 (stable) + model discovery
const GEMINI_API_BASE = 'https://generativelanguage.googleapis.com/v1';

// Optional preferred model; honored only if supported by your key
...
// Persistent store to avoid repeats (file-based, free)
const STORE_PATH =
...
// Optional debug: logs first 300 chars of raw model text on parse error
const DEBUG_MODEL_PREVIEW = String(process.env.DEBUG_MODEL_PREVIEW || '').toLowerCase() === 'true';

// Response JSON schema (helps force valid JSON)
const JSON_SCHEMA = {
  type: 'OBJECT',
  properties: {
    script: { type: 'STRING' },
  ...
  required: ['script', 'titles', 'description', 'hashtags']
};
...
{"script":"...","titles":{"a":"...","b":"...","c":"..."},"description":"...","hashtags":["#shorts","..."]}`;
  ...
  try {
    // 1) Generate
    let genText = await callGeminiAuto(userPrompt, { temperature: 0.9, maxOutputTokens: 800, schema: JSON_SCHEMA });
    let parsed = safeJsonParse(genText);

    // If still not JSON, retry once with a stricter instruction
    if (!parsed || !parsed.script) {
      if (DEBUG_MODEL_PREVIEW) {
        console.log('[Gemini][parse-fail][gen] preview:', String(genText).slice(0, 300));
      }
      const strictPrompt = `You must return ONLY a valid JSON object that matches this schema:
${JSON.stringify(JSON_SCHEMA)}
No markdown, no code fences, no commentary—JSON object only.

Task:
${userPrompt}`;
      genText = await callGeminiAuto(strictPrompt, { temperature: 0.85, maxOutputTokens: 800, schema: JSON_SCHEMA });
    ...
    if (wordCount < TARGET_MIN_WORDS || wordCount > TARGET_MAX_WORDS) {
      let refineText = await callGeminiAuto(
        `Refine the script to ${TARGET_MIN_WORDS}-${TARGET_MAX_WORDS} words. Keep the same meaning and style rules (hook on line 1, one sentence per line, micro-example, no lists/hashtags/emojis, CTA close).
Return JSON only: {"script":"..."}.

Original script:
${script}`,
        { temperature: 0.8, maxOutputTokens: 360, schema: { type: 'OBJECT', properties: { script: { type: 'STRING' }}, required: ['script'] } }
      );
      let refined = safeJsonParse(refineText);

      if (!refined?.script) {
        if (DEBUG_MODEL_PREVIEW) {
          console.log('[Gemini][parse-fail][refine] preview:', String(refineText).slice(0, 300));
        }
        const strictRefine = `Return ONLY this JSON shape: {"script":"..."} with ${TARGET_MIN_WORDS}-${TARGET_MAX_WORDS} words. No markdown. No commentary.

Original:
${script}`;
        refineText = await callGeminiAuto(strictRefine, { temperature: 0.8, maxOutputTokens: 360, schema: { type: 'OBJECT', properties: { script: { type: 'STRING' }}, required: ['script'] } });
        refined = safeJsonParse(refineText);
      }
    ...
    if (titlesMissing || titlesClash || !description) {
      let metaText = await callGeminiAuto(
...
Rules:
- Titles: ≤55 chars, curiosity/promise-driven, distinct from each other and from this list: ${JSON.stringify(recent.titles.slice(0, 20))}.
- Description: 1–2 lines, ≤170 chars, soft CTA, distinct from: ${JSON.stringify(recent.descriptions.slice(0, 20))}.
- No emojis or hashtags.`,
      ...
      if ((!meta.titles || !meta.description) && DEBUG_MODEL_PREVIEW) {
        console.log('[Gemini][parse-fail][meta] preview:', String(metaText).slice(0, 300));
      }
      const t = meta.titles || {};
    ...
    [titleA, titleB, titleC] = ensureThreeDistinct([titleA, titleB, titleC], topic, recent.titles.slice(0, 20));
    ...
    if (isNearDuplicateScript(script)) {
      let varyText = await callGeminiAuto(
...
Original:
${script}`,
        { temperature: 0.95, maxOutputTokens: 420, schema: { type: 'OBJECT', properties: { script: { type: 'STRING' }}, required: ['script'] } }
      );
      let v = safeJsonParse(varyText);

      if (!v?.script && DEBUG_MODEL_PREVIEW) {
        console.log('[Gemini][parse-fail][vary] preview:', String(varyText).slice(0, 300));
      }
      if (v?.script) {
...
async function callGeminiAuto(text, cfg = {}) {
  ...
  for (const m of candidates) {
    const full = m.name;              // e.g., "models/gemini-2.5-flash"
    const short = shortModelName(full); // e.g., "gemini-2.5-flash"
    try {
...
async function callGeminiRaw(modelShortName, text, opts = {}) {
  ...
  // If provided, attach response schema (both camelCase and snake_case for safety)
  if (opts.schema) {
    generationConfig.responseSchema = opts.schema;
    generationConfig.response_schema = opts.schema;
  }

  const payload = {
    contents: [{ role: 'user', parts: [{ text: String(text || '') }]}],
    generationConfig,
    // Extra belt-and-suspenders instruction
    systemInstruction: {
      role: 'system',
      parts: [{
        text: 'Always respond with a single valid JSON object that matches the provided schema. Do not include markdown, code fences, or any extra text.'
      }]
    }
  };
