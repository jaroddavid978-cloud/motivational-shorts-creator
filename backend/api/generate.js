...
router.get('/test-gemini', async (req, res) => {
  ...
  console.log('API Key prefix:', GEMINI_API_KEY?.substring(0, 8) || 'none');
  ...
  try {
    ...
    console.log('Testing URL:', testUrl.replace(GEMINI_API_KEY, 'HIDDEN'));
...
{"script":"...","titles":{"a":"...","b":"...","c":"..."},"description":"...","hashtags":["#shorts","..."]}`;
  ...
  try {
    ...
    if (!parsed || !parsed.script) {
      if (parsed === null) console.log('[DEBUG] Failed to parse JSON from:', genText?.substring(0, 200));
    ...
    return res.status(500).json({
      success: false,
      error: data?.error?.message || data?.message || err.message || 'Generation failed',
...
async function orderedCapableModels() {
  ...
  const sorted = capable.sort((a, b) => scoreModel(shortModelName(b.name)) - scoreModel(shortModelName(a.name)));
  return sorted;
}

async function callGeminiAuto(text, cfg = {}) {
  const candidates = await orderedCapableModels();
  if (!candidates.length) throw new Error('No Gemini models with generateContent are available');
  ...
  for (const m of candidates) {
    ...
    try {
      ...
      console.warn(`Gemini model failed (${short}) ->`, e.message);
      lastErr = e;
    }
  }
